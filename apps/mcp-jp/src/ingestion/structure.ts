...
export async function structureHtmlContent(
  ...
  const prompt = `You are a regulatory data extraction expert. Given the following content scraped from a Japanese regulatory website, extract structured data into the specified JSON format.
Content may be in Japanese. Extract and return all data in English.
...
Include the exact regulation reference where available (e.g., Food Sanitation Act Article 12, Food Labelling Standards).
