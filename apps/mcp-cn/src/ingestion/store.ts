...
async function storeIngredientRegulations(
  ...
  for (const entry of entries) {
    ...
    // Determine regulatory body from the entry or default based on product category
    const productCategories = (entry.product_categories as string[]) ?? [];
    const defaultBody = productCategories.some((c) => c.toLowerCase().includes('cosmetic'))
      ? 'NMPA'
      : 'SAMR';
    ...
    await supabase.from('ingredient_regulations').upsert(
      {
        ...
        jurisdiction: JURISDICTION,
        regulatory_body: (entry.regulatory_body as string) ?? defaultBody,
        ...
        conditions: {
          ...
          original_text_zh: entry.original_text_zh ?? null,
        } as unknown as Json,
...
async function storeLabellingRequirements(
  ...
  for (const entry of entries) {
    await supabase.from('labelling_requirements').upsert(
      {
        jurisdiction: JURISDICTION,
        regulatory_body: (entry.regulatory_body as string) ?? 'SAMR',
    ...
    await supabase.from('import_requirements').upsert(
      {
        ...
        regulatory_body: (entry.licensing_body as string) ?? 'GACC',
