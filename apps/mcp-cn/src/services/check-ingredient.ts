...
export const checkIngredientSchema = z.object({
  ingredient: z.string().describe('Ingredient name (Chinese or English), INCI name, or CAS number'),
...
export async function checkIngredient(args: CheckIngredientArgs) {
  ...
  if (!regulations || regulations.length === 0) {
    return {
      ...
      jurisdiction: 'CN',
      status: 'unknown',
      message: 'No regulatory data available for this ingredient in China. This does not mean the ingredient is permitted — consult the relevant regulatory authority.',
    };
  }
