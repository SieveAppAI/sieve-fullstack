 ...
 * - FSANZ Food Standards Code (foodstandards.gov.au) — JS-rendered content
 * - TGA permissible ingredients (tga.gov.au) — interactive search/filter
 */
...
function buildTaskForUrl(url: string): string {
  ...
  if (hostname.includes('foodstandards.gov.au')) {
    return `This is the FSANZ Food Standards Australia New Zealand website. Wait for the JavaScript-rendered content to fully load. Extract the complete text from the main content area, including: the standard/schedule title, all section numbers and their text, all tables with permitted substances and their limits, and any conditions or notes. If there are multiple parts or schedules, extract them all. Return the full text content. Do not truncate.`;
  }

  if (hostname.includes('tga.gov.au')) {
    return `This is the TGA (Therapeutic Goods Administration) website. Wait for the page to fully load. Find and extract all regulatory content including permitted/restricted ingredients, conditions of use, maximum doses, and any schedules or appendices. If there is a search or listing functionality, extract the visible data. Return the complete text content.`;
  }
