...
import { classifyJurisdiction } from './constants';
  ...
  const { error } = await supabase.from('regulatory_sources').upsert(
    {
      ...
      jurisdiction: classifyJurisdiction(page.url),
...
export async function storeStructuredData(
  ...
  // Get source ID and jurisdiction
  ...
  const jurisdiction = (source.jurisdiction as string) ?? classifyJurisdiction(sourceUrl);

  // Route data to appropriate tables
  // Cast to Record<string, unknown>[] since data comes from parsed JSON
  ...
  if (
    ...
    await storeIngredientRegulations(entries, source.id, jurisdiction);
  }
  ...
  if (
    ...
    const labellingEntries = (structuredData.labelling_requirements ??
      ...
      []) as unknown as Entry[];
    await storeLabellingRequirements(labellingEntries, source.id, jurisdiction);
  }
  ...
  if (
    ...
    const claimsEntries = (structuredData.claims_rules ??
      structuredData.entries ??
      []) as unknown as Entry[];
    await storeClaimsRules(claimsEntries, source.id, jurisdiction);
  }
  ...
  if (
    ...
    const importEntries = (structuredData.import_requirements ??
      structuredData.entries ??
      []) as unknown as Entry[];
    await storeImportRequirements(importEntries, source.id, jurisdiction);
  }
}
    ...
    await supabase.from('ingredient_regulations').upsert(
      {
        ...
        jurisdiction,
        regulatory_body: (entry.regulatory_body as string) ?? 'FSANZ',
    ...
    await supabase.from('import_requirements').upsert(
      {
        ...
        regulatory_body: (entry.licensing_body as string) ?? 'FSANZ',
