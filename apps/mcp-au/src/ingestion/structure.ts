...
export async function structureHtmlContent(
  ...
  const prompt = `You are a regulatory data extraction expert. Given the following content scraped from an Australia/New Zealand regulatory website, extract structured data into the specified JSON format.
...
The regulatory body may be one of: FSANZ (Food Standards Australia New Zealand), TGA (Therapeutic Goods Administration), AICIS (Australian Industrial Chemicals Introduction Scheme), Medsafe (New Zealand Medicines and Medical Devices Safety Authority), or NZ EPA (New Zealand Environmental Protection Authority).
