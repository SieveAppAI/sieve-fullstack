 ...
 * - old.sfda.gov.sa — Legacy JS-heavy SFDA portal
 * - store.gso.org.sa — GSO standards store (JS-rendered catalog)
 */
...
function buildTaskForUrl(url: string): string {
  ...
  if (hostname.includes('old.sfda.gov.sa')) {
    return `This is a legacy SFDA portal page. Wait for the JavaScript-rendered content to fully load. Extract all regulatory text including regulation titles, section headings, article numbers and text, schedules, and annexes. Return the full text content. Do not truncate.`;
  }

  if (hostname.includes('store.gso.org.sa')) {
    return `This is the GSO Standards Store. Search for food and cosmetic standards. For each standard found, extract the standard number, title, scope, status (active/withdrawn), and any referenced GSO standards. Return structured data for all standards found.`;
  }
