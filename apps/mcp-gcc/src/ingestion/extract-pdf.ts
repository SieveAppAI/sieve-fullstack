...
export async function extractPdfWithClaudeVision(
  ...
  const structured_data = JSON.parse(jsonText) as StructuredData;
...
function buildPdfExtractionPrompt(
  ...
  const basePrompt = `You are a regulatory data extraction expert. This PDF is from ${regulatoryBody} (SFDA/GSO — GCC). Document: ${description}.
