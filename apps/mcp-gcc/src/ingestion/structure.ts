...
export async function structureHtmlContent(
  ...
  const prompt = `You are a regulatory data extraction expert. Given the following content scraped from a GCC regulatory website (SFDA, GSO, MoIAT, NHRA, or other GCC regulator), extract structured data into the specified JSON format.

Additional GCC-specific guidance:
- For HALAL data: extract as claims_rules with claim_type "halal"
- For ARABIC LABELLING: extract as labelling_requirements, note language_requirements
- For country-specific import rules: include the specific country (SA/AE/BH/etc) in the licensing_body field
- SFDA often references GSO standards — preserve both SFDA and GSO references
