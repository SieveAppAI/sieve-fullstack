...
export async function structureHtmlContent(
  ...
  const prompt = `You are a regulatory data extraction expert. Given the following content scraped from a United States regulatory website, extract structured data into the specified JSON format.
...
Include the exact regulation reference where available (e.g., 21 CFR 172.515, FALCPA Section 403(w)).
